export const socialismContent = {
  title: "Tư tưởng Hồ Chí Minh về chủ nghĩa xã hội",
  subtitle: "Ba nội dung cơ bản định hình tư tưởng xã hội chủ nghĩa Việt Nam",
  
  sections: [
    {
      id: "a",
      title: "Quan niệm của Hồ Chí Minh về chủ nghĩa xã hội",
      subtitle: "Kết hợp Mác-Lênin với điều kiện Việt Nam",
      icon: "🎯",
      color: "from-blue-600 to-blue-800",
      accentColor: "from-blue-100 to-blue-200",
      content: [
        "Hồ Chí Minh tiếp nhận tinh hoa của chủ nghĩa Mác-Lênin nhưng luôn đặt trong bối cảnh cụ thể của dân tộc Việt Nam: chủ nghĩa xã hội là mục tiêu tối hậu của cách mạng dân tộc dân chủ và sau đó là cách mạng xã hội chủ nghĩa.",
        "Bác nhấn mạnh: chủ nghĩa xã hội không chỉ là sở hữu công cộng về tư liệu sản xuất, mà còn là xã hội công bằng, dân chủ, văn minh, người dân được tự do phát triển toàn diện cả về vật chất lẫn tinh thần.",
        "Quá trình tiến lên xã hội chủ nghĩa phải phù hợp với trình độ phát triển kinh tế — xã hội của quốc gia (không áp đặt mô hình máy móc), kết hợp hiện đại hóa lực lượng sản xuất với nâng cao văn hoá, đạo đức, giáo dục.",
        "Vai trò lãnh đạo của Đảng Cộng sản, nhà nước của dân, do dân, vì dân; kết hợp đấu tranh giai cấp với đoàn kết dân tộc; thực hiện hoà bình, hợp tác quốc tế."
      ],
      summary: {
        title: "Tóm tắt:",
        points: [
          "Chủ nghĩa xã hội theo HCM = mục tiêu lịch sử, kết hợp Mác-Lênin với điều kiện Việt Nam",
          "Không chỉ sở hữu công cộng mà là xã hội công bằng, dân chủ, văn minh, phát triển toàn diện con người",
          'Tiến trình phải "vừa làm vừa chỉnh", phù hợp trình độ phát triển, coi trọng giáo dục, đạo đức, văn hoá',
          "Đảng, Nhà nước và nhân dân là nhân tố quyết định; tinh thần quốc tế chủ nghĩa và hoà bình"
        ]
      },
      annotations: [
        "(1) Hồ Chí Minh: Toàn tập, Sđd, t.12, tr.415; t.10, tr.390."
      ],
      quote: "Nói một cách tóm tắt, mộc mạc, chủ nghĩa xã hội trước hết nhằm làm cho nhân dân lao động thoát nạn bần cùng, làm cho mọi người có công ăn việc làm, được ấm no và sống một đời hạnh phúc”, là làm sao cho dân giàu, nước mạnh ? (1)",
      quoteAuthor: "Chủ tịch Hồ Chí Minh",
      image: "/images/giai-phong-dan-toc/cach-mang-vo-san.jpg"
    },
    {
      id: "b", 
      title: "Con đường đi lên chủ nghĩa xã hội là một tất yếu của lịch sử",
      subtitle: "Quy luật lịch sử và con đường phù hợp",
      icon: "📈",
      color: "from-green-600 to-green-800",
      accentColor: "from-green-100 to-green-200",
      content: [
        "Hồ Chí Minh tin vào quy luật lịch sử: từ xã hội có giai cấp tiến tới xã hội không còn giai cấp — chủ nghĩa xã hội là bước phát triển tất yếu mà khoa học lịch sử (theo quan điểm Mác) chỉ ra.",
        "Ở Việt Nam, ông cho rằng sau khi giành độc lập dân tộc (tiêu diệt chủ nghĩa thực dân, phong kiến), con đường tất yếu phải là xây dựng xã hội mới — xã hội chủ nghĩa — để giải phóng dân tộc khỏi nghèo nàn, tụt hậu.",
        'Tuy nhiên, "tất yếu" ở đây không đồng nghĩa với "tự động" hay "ngay lập tức": HCM nhấn mạnh yếu tố chủ động - công tác lãnh đạo, tổ chức, sự giác ngộ của nhân dân, và bước đi phù hợp (giai đoạn chuyển tiếp: cách mạng dân tộc dân chủ → phát triển lực lượng sản xuất → xã hội chủ nghĩa).',
        "Ông cũng nhấn mạnh yếu tố khách quan (quy luật lịch sử, phát triển lực lượng sản xuất) và chủ quan (lãnh đạo đúng, chính sách đúng) cùng quyết định."
      ],
      summary: {
        title: "Tóm tắt:",
        points: [
          "HCM coi tiến lên XHCN là quy luật lịch sử, là mục tiêu tất yếu sau độc lập dân tộc",
          '"Tất yếu" nhưng phải có con đường phù hợp: kết hợp điều kiện khách quan với quyết tâm chủ quan',
          "Cần các bước chuyển tiếp (xây dựng kinh tế, văn hoá, giáo dục) chứ không thể ép buộc tức thời"
        ]
      },
      annotations: [
        "(2) C.Mác và Ph.Ăngghen: Toàn tập, Sđd, t.4, tr.613"
      ],
      quote: "Sự sụp đổ của giai cấp tư sản và thắng lợi của giai cấp vô sản là tất yếu như nhau.(2)",
      quoteAuthor: "Karl Marx",
      image: "/images/giai-phong-dan-toc/dang-cong-san-viet-nam-lanh-dao.jpg"
    },
    {
      id: "c",
      title: "Một số đặc trưng cơ bản của xã hội chủ nghĩa",
      subtitle: "Bốn đặc trưng nổi bật",
      icon: "🏛️",
      color: "from-purple-600 to-purple-800",
      accentColor: "from-purple-100 to-purple-200",
      content: [
        "Dựa trên tư tưởng HCM, xã hội chủ nghĩa có các đặc trưng nổi bật sau (lưu ý là HCM kết hợp lý luận Mác-Lênin với thực tiễn Việt Nam nên có sắc thái riêng):"
      ],
      features: [
        {
          title: "Sở hữu tập thể",
          description: "Sở hữu chủ yếu là công cộng / tập thể về tư liệu sản xuất (nhà nước, hợp tác xã, doanh nghiệp xã hội chủ nghĩa) — nhằm xóa bỏ bóc lột.",
          icon: "🏭",
          category: "kinh-te"
        },
        {
          title: "Kinh tế vì con người",
          description: "Kinh tế phát triển vì con người: nâng cao lực lượng sản xuất, phát triển công-nông nghiệp đồng đều, chú trọng khoa học-kỹ thuật.",
          icon: "👥",
          category: "kinh-te"
        },
        {
          title: "Công bằng xã hội",
          description: "Công bằng xã hội: thu hẹp chênh lệch, chăm sóc an sinh, giáo dục-y tế phổ cập để mọi người có cơ hội phát triển.",
          icon: "⚖️",
          category: "chinh-tri"
        },
        {
          title: "Dân chủ xã hội chủ nghĩa",
          description: "Dân chủ thực chất trong khuôn khổ lãnh đạo của Đảng, Nhà nước do dân bầu và chịu trách nhiệm trước dân; người lao động tham gia quản lý.",
          icon: "🗳️",
          category: "chinh-tri"
        },
        {
          title: "Văn hoá - đạo đức",
          description: "Văn hoá, đạo đức xã hội chủ nghĩa: đề cao nhân cách, trí tuệ, lao động sáng tạo, tinh thần đoàn kết; giáo dục nhân dân là then chốt.",
          icon: "📚",
          category: "van-hoa"
        },
        {
          title: "Phát triển toàn diện",
          description: "Phát triển toàn diện con người: vừa phát triển vật chất, vừa chăm lo tinh thần, bản sắc văn hoá dân tộc phải được bảo lưu và phát huy.",
          icon: "🌟",
          category: "van-hoa"
        },
        {
          title: "Quốc tế chủ nghĩa",
          description: "Chủ nghĩa quốc tế và hoà bình: hợp tác quốc tế, ủng hộ phong trào giải phóng dân tộc, chống chủ nghĩa đế quốc.",
          icon: "🌍",
          category: "chinh-tri"
        },
        {
          title: "Chủ thể xây dựng",
          description: "Chủ nghĩa xã hội là công trình tập thể của nhân dân dưới sự lãnh đạo của Đảng Cộng sản. Nhân dân là chủ thể, là lực lượng quyết định tốc độ xây dựng và sự vững mạnh của chủ nghĩa xã hội.",
          icon: "🏛️",
          category: "chu-the-xay-dung"
        }
      ],
      summary: {
        title: "Tóm tắt:",
        points: [
          "Sở hữu tập thể về tư liệu sản xuất; phát triển lực lượng sản xuất vì con người",
          "Công bằng xã hội + an sinh, giáo dục-y tế phổ cập",
          "Dân chủ trong khung lãnh đạo của Đảng; người dân tham gia quản lý",
          "Đề cao văn hoá-đạo đức, phát triển toàn diện con người; hoà bình và quốc tế chủ nghĩa"
        ]
      },
      annotations: [
        "Các đặc trưng này được HCM đúc kết từ nghiên cứu các mô hình XHCN trên thế giới",
        "Đặc biệt chú trọng yếu tố văn hóa dân tộc trong xây dựng XHCN",
        "Quan điểm này được thể hiện rõ trong Cương lĩnh xây dựng đất nước thời kỳ quá độ"
      ],
      quote: "Xã hội chủ nghĩa phải là xã hội của nhân dân, do nhân dân, vì nhân dân.",
      quoteAuthor: "Chủ tịch Hồ Chí Minh",
      image: "/images/giai-phong-dan-toc/dai-doan-ket-dan-toc.jpg"
    }
  ]
};
